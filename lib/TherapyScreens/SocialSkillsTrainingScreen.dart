...
class _SocialSkillsTrainingScreen extends State<SocialSkillsTrainingScreen> {
  ...
  Future<void> GetReviewsList() async {
    final response = await Userapi.getreviewlist("Social Skills Training");
    if (response != null) {
      setState(() {
      ...
      child: Column(
        ...
        children: [
          ...
          if (reviews.length > 3)
            InkResponse(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (context)=>ReviewListScreen(pageSource: "Social Skills Training",)));
              },
              child: Padding(
