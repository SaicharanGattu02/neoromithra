...
class _ParentChildRelationshipCounsellingScreenState extends State<ParentChildRelationshipCounsellingScreen> {
  ...
  Future<void> GetReviewsList() async {
    final response = await Userapi.getreviewlist("Parent-Child Relationship");
    if (response != null) {
      setState(() {
      ...
      child: Column(
        ...
        children: [
          ...
          if (reviews.length > 3)
            InkResponse(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (context)=>ReviewListScreen(pageSource: "Parent-Child Relationship",)));
              },
              child: Padding(
