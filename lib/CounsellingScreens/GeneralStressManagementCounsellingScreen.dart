...
class _GeneralStressManagementCounsellingScreenState extends State<GeneralStressManagementCounsellingScreen> {
  ...
  Future<void> GetReviewsList() async {
    final response = await Userapi.getreviewlist("General stress management");
    if (response != null) {
      setState(() {
      ...
      child: Column(
        ...
        children: [
          ...
          if (reviews.length > 3)
            InkResponse(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (context)=>ReviewListScreen(pageSource: "General stress management",)));
              },
              child: Padding(
